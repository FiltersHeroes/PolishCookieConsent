...
var cm6 = (function (exports) {
  ...
  */
  ...
  function onChange(editor, callback) {
    editor.dispatch({
  ...
  function onSave(editor, callback) {
    editor.save = () => callback(editor);
    editor.dispatch({
      effects: StateEffect.appendConfig.of(keymap.of([{
        key: "Mod-s",
        run: () => {
  ...
  function initCustomSearch(editorView, searchInput, counter, nextBtn, prevBtn) {
    let matches = [];
    let currentIndex = -1;
  ...
  exports.initCustomSearch = initCustomSearch;
